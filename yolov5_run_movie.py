    ...
    if ret:
        ...
        if len(results.xyxy[0]) > 0:
            for detection in results.xyxy[0]:
                if detection[4] > 0.5 :  # 確信度が0.7以上でクラスが"person"の場合のみ処理を行う:  # 確信度が0.9以上の場合のみ処理を行う
        ...
        objects = []
        for detection in results.xyxy[0]:
            if detection[4] > 0.5 and model.names[int(detection[5])] == "apple":  # 確信度が0.7以上でクラスが"person"の場合のみ処理を行う:  # 確信度の設定
                x1, y1, x2, y2, conf, cls = detection.tolist()
                obj = {
                        'detection':[
                            {
